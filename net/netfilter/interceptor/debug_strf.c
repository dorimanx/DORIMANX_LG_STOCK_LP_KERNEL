...
const char *debug_str_hexbuf(DEBUG_STRBUF_TYPE buf, const void *data, int len)
{
	int p_len;
	char *p;
...
const char *debug_strbuf_sshrender(DEBUG_STRBUF_TYPE dsb,
				   int (*ssh_renderer) (unsigned char *, int,
							int, void *),
				   void *render_datum)
{
...
static void
longest_zero_pairs(const unsigned char address[16], int *start_p, int *length_p)
{
...
static int
format_ip6_address(const unsigned char address[16],
		   int zero_start, int zero_length, char *p, int p_len)
{
	int address_index;
	int i = 0;
	...
	for (address_index = 0; address_index <= 14;) {
		...
		if (address_index == zero_start && zero_length != 0) {
			...
			if (a1 != 0) {
				i += snprintf(p + i, p_len - 1, "%x%.2x", a1,
					      a2);
			} else {
...
const char *debug_strbuf_ipaddress(DEBUG_STRBUF_TYPE buf,
...
int format_ipaddress(char *buf, int len, const unsigned char address[16])
{
	int zero_start;
	int zero_length;
	int used;
	...
	if (zero_start == 0 && zero_length == 12) {
		...
		used =
		    format_ip6_address(address, zero_start, zero_length, buf,
				       len);
	}
