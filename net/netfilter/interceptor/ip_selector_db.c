...
void ip_selector_db_init(struct IPSelectorDb *db)
{
	memset(db, 0, sizeof *db);
}

int
ip_selector_db_entry_check(const struct IPSelectorDbEntry *entry,
			   uint32_t length)
{
	...
	return
	    ip_selector_match_validate_selector_group(selector_group,
						      bytecount);
}

void
ip_selector_db_entry_add(struct IPSelectorDb *db,
			 int part,
			 struct IPSelectorDbEntry *entry, int precedence)
{
	...
	*loc = entry;
	...
	DEBUG_DUMP(db,
		   ...
		   db,
		   part, entry->id, entry->action, entry->priority, precedence);
}

struct IPSelectorDbEntry *ip_selector_db_entry_remove(struct IPSelectorDb *db,
						      int part, uint32_t id)
{
	...
	if (removed != NULL) {
		DEBUG_DUMP(db,
			   ...
			   db,
			   part,
			   removed->id, removed->action, removed->priority);
	} else {
		DEBUG_FAIL(db, "Remove failed for db part %d id %d.", part, id);
	}
...
int
ip_selector_db_lookup(struct IPSelectorDb *db,
		      int part, const struct IPSelectorFields *fields)
{
	...
	DEBUG_LOW(lookup,
		  ...
		  fields,
		  part,
		  debug_str_ip_selector_fields(DEBUG_STRBUF_GET(),
	...
	while (entry != NULL) {
		...
		if (ip_selector_match_fields_to_group(selector_group, fields)) {
			...
			DEBUG_LOW(lookup, "Fields %p matched entry %p.", fields,
				  entry);
			break;
		}
