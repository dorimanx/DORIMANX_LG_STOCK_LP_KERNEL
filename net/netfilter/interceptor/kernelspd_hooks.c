...
int
make_spd_lookup(int hook,
		const struct net_device *in,
		const struct net_device *out,
		struct udphdr *udph,
		struct IPSelectorFields *fields, bool non_initial_fragment)
{
	int spd_id;
	const char *spd_name;
...

	{
		...
		default:
			DEBUG_FAIL(hook,
				   ...
				   result);
			HOOK_DEBUG(FAIL, result, hook, spd_name, in_name,
				   out_name, fields);
			verdict = NF_DROP;
			break;
		}
		...
		if (verdict == NF_DROP) {
			HOOK_DEBUG(HIGH, result, hook, spd_name, in_name,
				   out_name, fields);
		} else if (spd_id >= 0) {
			HOOK_DEBUG(MEDIUM, result, hook, spd_name, in_name,
				   out_name, fields);
		} else {
			HOOK_DEBUG(LOW, result, hook, spd_name, in_name,
				   out_name, fields);
		}
...
static unsigned int
hook_ipv4(unsigned int hooknum,
	  struct sk_buff *skb,
	  const struct net_device *in,
	  const struct net_device *out, int (*okfn) (struct sk_buff *))
{
	...
	return
	    make_spd_lookup(hooknum,
			    in, out, udph, &fields, non_initial_fragment);
}

static void *parse_ip6_headers(struct sk_buff *skb,
			       void *tmpbuf,
...
static unsigned int
hook_ipv6(unsigned int hooknum,
	  struct sk_buff *skb,
	  const struct net_device *in,
	  const struct net_device *out, int (*okfn) (struct sk_buff *))
{
	...
	if (iph != NULL) {
		...
		udph =
		    parse_ip6_headers(skb,
				      portsbuf,
				      &non_initial_fragment, &nexthdr);
...
int spd_hooks_init(void)
{
	...
	if (initialised == 0) {
		...
		if (result == 0) {
			...
			DEBUG_FAIL(hook, "nf_register_hooks() failed %d.",
				   result);
		}
	}
